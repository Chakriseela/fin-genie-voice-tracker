...
const AddExpense = ({ email, setExpenses, setTotalExpenses }) => {
  const [expense, setExpense] = useState({ title: '', amount: '', date: '', category: 'others' });
  ...
  const handleSubmit = async (e) => {
    ...
    const newExpense = { email, ...expense, amount: parseFloat(expense.amount) };

    try {
      setLoading(true);
      const response = await fetch('https://fin-genie-voice-tracker.onrender.com/api/expenses/add', {
        method: 'POST',
      ...
      if (response.ok && data.expense) {
        ...
        const updatedResponse = await fetch(`https://fin-genie-voice-tracker.onrender.com/api/expenses?email=${email}`);
  ...
  return (
    <form onSubmit={handleSubmit} className="add-expense-container animate-form">
