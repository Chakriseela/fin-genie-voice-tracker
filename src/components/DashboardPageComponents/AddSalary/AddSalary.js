...
const AddSalary = ({ email, setIncomeList, setTotalIncome }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch('https://fin-genie-voice-tracker.onrender.com/api/income/add', {
        method: 'POST',
      ...
      if (response.ok && data.income) {
        ...
        const updatedResponse = await fetch(`https://fin-genie-voice-tracker.onrender.com/api/income?email=${email}`);
