...
import { PieChart, Pie, BarChart, Bar, LineChart, Line, Tooltip, XAxis, YAxis, CartesianGrid, Cell, ResponsiveContainer } from 'recharts';
...
const FinGenie = ({ email }) => {
  ...
  const fetchExpenses = async () => {
    try {
      const res = await fetch(`https://fin-genie-voice-tracker.onrender.com/api/expenses?email=${email}`);
  ...
  const fetchIncome = async () => {    // New API Call
    try {
      const res = await fetch(`https://fin-genie-voice-tracker.onrender.com/api/income?email=${email}`);
      const data = await res.json();
  ...
  // Common Functions
  const sumAmount = (items) => items.reduce((acc, item) => acc + item.amount, 0);
  ...
  const getIncomeByMonth = (month) => sumAmount(income.filter(e => getMonth(e.date) === month));
  const getExpensesByMonth = (month) => sumAmount(expenses.filter(e => e.category !== 'income' && getMonth(e.date) === month));
  ...
  const exportToPDF = () => {
    ...
    messages.forEach((msg, i) => {
      doc.text(`${msg.sender === 'user' ? 'You' : 'FinGenie'}: ${msg.text}`, 10, 20 + i * 10);
    });
  ...
  // Graph Responses
  const showPieChart = () => respond("📊 Here's your expense category breakdown:", 'pie');
  ...
  const parseUserQuery = async (text) => {
    ...
    const commands = [
      { keywords: ['total income'], action: () => respond(`Total income is ₹${getTotalIncome()}`) },
      { keywords: ['total expenses'], action: () => respond(`Total expenses are ₹${getTotalExpenses()}`) },
      { keywords: ['income last month'], action: () => respond(`Income last month was ₹${getIncomeByMonth(new Date().getMonth())}`) },
      { keywords: ['expenses last month'], action: () => respond(`Expenses last month were ₹${getExpensesByMonth(new Date().getMonth())}`) },
      { keywords: ['income current month'], action: () => respond(`Income this month is ₹${getIncomeByMonth(new Date().getMonth() + 1)}`) },
      { keywords: ['expenses current month'], action: () => respond(`Expenses this month are ₹${getExpensesByMonth(new Date().getMonth() + 1)}`) },
    ...
    const foundCommand = commands.find(cmd => cmd.keywords.some(k => lower.includes(k)));
      ...
      {isOpen && (
        <div className="fingenie-chat-window">
              ...
              <div key={i} className={`fingenie-msg ${msg.sender}`}>
                ...
                {msg.showChart === 'pie' && (
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      <Pie dataKey="amount" data={expenses.filter(e => e.category !== 'income')} cx="50%" cy="50%" outerRadius={80} label>
          ...
          <div className="fingenie-input">
            ...
            <button onClick={handleVoice}>{listening ? '🎙️ Listening...' : '🎤 Voice'}</button>
          </div>
        </div>
      )}
      <button className="fingenie-toggle-btn" onClick={() => setIsOpen(!isOpen)}>
        {isOpen ? '✖' : '💬'}
      </button>
    </div>
  );
