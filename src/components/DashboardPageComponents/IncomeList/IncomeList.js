...
const IncomeList = ({ incomeList = [], fetchIncome }) => {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`https://fin-genie-voice-tracker.onrender.com/api/income/delete/${selectedId}`, {
        method: "DELETE",
      });
          ...
          {incomeList.map((item) => (
            <li key={item._id} className="income-item">
              ...
              <div className="income-right">
                <span className="income-amount">₹{Number(item.amount).toLocaleString('en-IN')}</span>
                <span className="income-date">{new Date(item.date).toLocaleDateString()}</span>
                <button
