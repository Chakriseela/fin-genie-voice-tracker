...
const VoiceInputButton = ({
  ...
  const handleConversation = async (text) => {
    ...
    if (conversationStateRef.current === "incomeTitle") {
      if (!text.trim()) return speak("Please provide a valid title for the income.", startRecording);
    ...
    if (conversationStateRef.current === "incomeDate") {
      ...
      if (!title || !amount || !email) return speak("Some data is missing. Please try again.", resetConversation);
      ...
      try {
        const res = await fetch("https://fin-genie-voice-tracker.onrender.com/api/income/add", {
          method: "POST",
    ...
    if (conversationStateRef.current === "expenseCategory") {
      if (!text.trim()) return speak("Please say a valid category.", startRecording);
    ...
    if (conversationStateRef.current === "expenseTitle") {
      if (!text.trim()) return speak("Please provide a valid title.", startRecording);
      updateTempData({ title: text });
    ...
    if (conversationStateRef.current === "expenseDate") {
      ...
      try {
        const res = await fetch("https://fin-genie-voice-tracker.onrender.com/api/expenses/add", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
    ...
    speak("Say something like 'add income of 5000' or 'add expense of 1000'", startRecording);
  };
