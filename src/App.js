...
// 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
...
const App = () => {
  ...
  const fetchExpenses = async () => {
    try {
      const res = await fetch(`https://fin-genie-voice-tracker.onrender.com/api/expenses?email=${userEmail}`);
  ...
  const fetchIncome = async () => {
    try {
      const res = await fetch(`https://fin-genie-voice-tracker.onrender.com/api/income?email=${userEmail}`);
      const data = await res.json();
              ...
              currentUser ? (
                <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
                  ...
                  <div className="Main-Dashboard">
                    ...
                    <OverviewCards totalIncome={totalIncome} totalExpenses={totalExpenses} expenses={expenses} />
                    <IncomeCard totalIncome={totalIncome} />
                    <IncomeList incomeList={incomeList} fetchIncome={fetchIncome} />
                    <div className="flex justify-center">
                      ...
                      />
                    </div><ChartsSection expenses={expenses} email={userEmail} />
                    <div className="forms-wrapper">
                      <div className="add-income-container">
                        <AddSalary email={userEmail} setIncomeList={setIncomeList} setTotalIncome={setTotalIncome} />
                      </div>
                      <div className="add-expense-container">
                        <AddExpense email={userEmail} setExpenses={setExpenses} setTotalExpenses={setTotalExpenses} />
                      </div>
                    </div>
